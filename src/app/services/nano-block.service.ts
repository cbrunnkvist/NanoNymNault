...
export class NanoBlockService {
  ...
  async generateChange(walletAccount, representativeAccount, ledger = false) {
    ...
    const walletAccountPublicKey = this.util.account.getAccountPublicKey(
      walletAccount.id,
    );
    ...
    if (ledger) {
      ...
      try {
        ...
        await this.ledgerService.updateCache(
          walletAccount.index,
          toAcct.frontier,
        );
        const sig = await this.ledgerService.signBlock(
          walletAccount.index,
          ledgerBlock,
        );
  ...
  async generateSend(walletAccount, toAccountID, rawAmount, ledger = false) {
    ...
    if (!fromAccount)
      throw new Error(
        `Unable to get account information for ${walletAccount.id}`,
      );
    ...
    if (ledger) {
      ...
      try {
        ...
        this.notifications.sendWarning(
          err.message || `Transaction denied on Ledger device`,
        );
        return;
      }
    } else {
  ...
  async signOfflineBlock(
    walletAccount: WalletAccount,
    block: StateBlock,
    prevBlock: StateBlock,
    type: TxType,
    genWork: boolean,
    multiplier: number,
    ledger = false,
    ...
    if (ledger) {
      ...
      if (type === TxType.send) {
        ledgerBlock = {
          ...
          recipient: this.util.account.getPublicAccountID(
            this.util.hex.toUint8(block.link),
          ),
        };
      ...
      try {
        ...
        if (!openEquiv) {
          try {
            ...
            await this.ledgerService.updateCacheOffline(
              walletAccount.index,
              prevBlock,
            );
          } catch (err) {
  ...
  async validateAccount(accountInfoUntrusted, accountPublicKey) {
    ...
    if (
      ...
      accountInfoUntrusted.frontier === this.zeroHash
    ) {
      if (
        accountInfoUntrusted.balance &&
        accountInfoUntrusted.balance !== "0"
      ) {
        throw new Error(
          `Frontier not set, but existing account balance is nonzero`,
        );
      }

      if (accountInfoUntrusted.representative) {
        throw new Error(
          `Frontier not set, but existing account representative is set`,
        );
      }
    ...
    const frontierBlockResponseUntrusted = await this.api.blocksInfo([
      accountInfoUntrusted.frontier,
    ]);
    ...
    if (!frontierBlockDataUntrusted)
      throw new Error(`Unable to load frontier block data`);

    frontierBlockDataUntrusted.contents = JSON.parse(
    ...
    const isFrontierBlockMatchingAccountInfo =
      frontierBlockDataUntrusted.contents.balance ===
        accountInfoUntrusted.balance &&
      frontierBlockDataUntrusted.contents.representative ===
        accountInfoUntrusted.representative;
    ...
    if (frontierBlockDataUntrusted.contents.type !== "state") {
      throw new Error(
        `Frontier block wasn't a state block, which shouldn't be possible`,
      );
    }

    const isComputedBlockHashMatchingAccountFrontierHash =
      this.util.hex.fromUint8(
        this.util.nano.hashStateBlock(frontierBlockDataUntrusted.contents),
      ) === accountInfoUntrusted.frontier;
    ...
    if (frontierBlockDataUntrusted.subtype === "epoch") {
      const isEpochV2BlockSignatureValid = nanocurrencyWebTools.verifyBlock(
      ...
      if (isEpochV2BlockSignatureValid !== true) {
        throw new Error(
          `Node provided an untrusted frontier block that is an unsupported epoch`,
        );
      }
    } else {
      const isFrontierBlockSignatureValid = nanocurrencyWebTools.verifyBlock(
        accountPublicKey,
        frontierBlockDataUntrusted.contents,
      );
      ...
      if (isFrontierBlockSignatureValid !== true) {
        throw new Error(
          `Node provided an untrusted frontier block that was signed by someone else`,
        );
      }
    }
  }
  ...
  getRandomRepresentative() {
    return this.representativeAccounts[
      Math.floor(Math.random() * this.representativeAccounts.length)
    ];
  }
